...
function renderMap(){
  ...
  events.slice(0,400).forEach(e=>{
    const locs=Array.isArray(e.locations)?e.locations.filter(l=>hasCoord(l?.lat,l?.lng)):[];
    if(locs.length){
  ...
  const hasPrecise=e=>(Array.isArray(e.locations)&&e.locations.some(l=>hasCoord(l?.lat,l?.lng)))||hasCoord(e.lat,e.lng);
  const precise=visibleEvents.filter(hasPrecise);
  const mapPointCount=precise.reduce((n,e)=>{const locs=Array.isArray(e.locations)?e.locations.filter(l=>hasCoord(l?.lat,l?.lng)):[];return n+(locs.length||1)},0);
