...
async function speakBriefing(){if(!('speechSynthesis'in window)||!state.briefing?.text)return;speechSynthesis.cancel();state.speaking=true;const chunks=sentenceChunks(state.briefing.text),sel=$('#voiceSelect'),voice=state.voices[Number(sel.value)]||null;for(const chunk of chunks){if(!state.speaking)break;await new Promise(resolve=>{const u=new SpeechSynthesisUtterance(chunk);if(voice)u.voice=voice;u.rate=.95;u.pitch=1;u.onend=resolve;u.onerror=resolve;speechSynthesis.speak(u)})}}
$('#briefBtn').onclick=speakBriefing;$('#stopBtn').onclick=()=>{state.speaking=false;if('speechSynthesis'in window)speechSynthesis.cancel()};
